...
public class DataManyToMany implements DatabaseCreator {

    private final Config config;

    private Connection connection;

    private static final DataManyToMany INSTANCE = new DataManyToMany();

    /**
     * Create and init config and get values from parser.properties.
     */
    public DataManyToMany() {
        this.config = new Config();
        config.init();
        getConnection();
    }

    public static DataManyToMany getInstance() {
        return INSTANCE;
    }
    ...
    public void createDB() {
        try {
            ...
            try (Statement st = connection.createStatement()) {
                String sql = "CREATE DATABASE hb_many_to_many";
    ...
    public boolean checkTableExist() {
        ...
        try (Statement st = connection.createStatement()) {
            ...
            if (rs.next()) {
                ...
                st.addBatch("create table student (id serial primary key, first_name varchar(45),"
                        + "last_name varchar(45), email varchar(45),"
                        + "instructor_detail_id int references instructor_detail(id))");
                st.addBatch("create table course_student (course_id int references course(id),"
    ...
    public void deleteTable() {
        try (Statement st = connection.createStatement()) {
            ...
            st.addBatch("DROP TABLE student CASCADE");
            st.addBatch("DROP TABLE course_student CASCADE");
            st.executeBatch();
